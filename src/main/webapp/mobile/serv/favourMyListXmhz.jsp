...
<%--<footer class="ui-footer ui-footer-btn">--%>
    <%--<ul class="ui-tiled ui-border-t">--%>
      ...
      <%--<li data-href="servHelpCreateXmhz.jsp?category=<%= category %>&accountNum=<%= accountNum %>"><i class="fa fa-pencil-square-o"></i>发帖</li>--%>
    <%--</ul>--%>
<%--</footer>--%>
        ...
        <ul class="ui-tab-content" style="width:200%">
            <li class="current">
                <div class="labelheader_xmhz">
                    <img src="../img/icon_comments.png" style="margin:0px 5px 0px 0px;"/>我的项目
                </div>
...
<script>
    ...
    //页面加载
    getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>","#myPost","onload",appealTplXmhz2,null,"loadPhotoSwipe");
    ...
    $(document).on('tap','.more',function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                var result=$.tpl(appealTplXmhz2,data);
    ...
    (function (){
        ...
        tab.on('scrollEnd', function(curPage) {
            if($('#myPost').children().length == 0) {
                getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>","#myPost","",appealTplXmhz2,null,"loadPhotoSwipe");
            }
