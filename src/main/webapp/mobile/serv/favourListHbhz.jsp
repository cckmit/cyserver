...
<%--<footer class="ui-footer ui-footer-btn">--%>
  <%--<ul class="ui-tiled ui-border-t">--%>
    ...
    --%>
    <%--<li data-href="favourListHbhz.jsp?isWhat=1&category=<%= category %>&accountNum=<%= accountNum %>" <% if("1".equals(isWhat)){%>class="current"<%}%>><i class="fa fa-home"></i>全部</li>--%>
    <%--<li data-href="favourMyListHbhz.jsp?isWhat=2&category=<%= category %>&accountNum=<%= accountNum %>" <% if("2".equals(isWhat)){%>class="current"<%}%>><i class="fa fa-user"></i>我的</li>--%>
  	<%--<li data-href="servHelpCreateHbhz.jsp?category=<%= category %>&accountNum=<%= accountNum %>"><i class="fa fa-pencil-square-o"></i>发帖</li>--%>
  <%--</ul>--%>
<%--</footer>--%>
<%--##############################################################################################################################drx start######################################--%>
...
<script>
  ...
  Zepto(function($){
  	 ...
  	 //页面加载
    getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>",".thread-list","onload",appealTplHbhz,null,"loadPhotoSwipe");
    ...
    var dropload = $('.inner').dropload({
      ...
      loadUpFn : function(me){
        getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>&cyServ.currentRow=0&cyServ.incremental=" + $(".thread-list > li").length,".thread-list","update",appealTplHbhz,me);
      },
      loadDownFn : function(me){
  		...
  		{
  			getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>&cyServ.currentRow=" + $(".thread-list > li").length,".thread-list","more",appealTplHbhz,me);
  		}
      }
    });
  ...
  $(document).on('tap','.show-bot-menu',function(){
    ...
    $(document).on('tap','.choose-city .ui-label',function(){
      ...
      getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=" + city,".thread-list","onload",appealTplHbhz,null,"loadPhotoSwipe");
