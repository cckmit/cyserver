...
<script>
    ...
    Zepto(function ($) {
        ...
        $.ajax({
            ...
            url: "<%=path%>/mobile/serv/znfxAction!doNotNeedSessionAndSecurity_getMobileNewsType_new.action?serviceId=<%=serviceId %>&u=<%=accountNum%>",
            dataType: 'json',
            ...
            success: function(data){
                ...
                if(data.leveList.length==0){
                	...
                	var params = "?category="+category+"&serviceId=<%=serviceId %>&start="+start+"&rows="+rows+"&u=<%=accountNum%>";
                    ...
                    getPageData("<%=path%>/mobile/serv/znfxAction!doNotNeedSessionAndSecurity_getNewsListByMobileType_new.action"+params,".ui-list","onload",newsList,null,"showMore");
                    sigleCategory();
                    ...
                    $('.inner').append(htmlTpl);
                    getPageData("<%=path%>/mobile/serv/znfxAction!doNotNeedSessionAndSecurity_getMobileNewsType_new.action?serviceId=<%=serviceId %>&u=<%=accountNum%>", ".ui-tab", "", tabNav, null, "tabNav");
                    multiCategory();
                }
            },
        ...
        function sigleCategory(){
            var dropload = $('.inner').dropload({
                ...
                loadUpFn : function(me){
                	var temUrl = "<%=path%>/mobile/serv/znfxAction!doNotNeedSessionAndSecurity_getNewsListByMobileType_new.action"+"?category="+category+"&serviceId=<%=serviceId %>&start=0&rows=10"+"&u=<%=accountNum%>";
                    getPageData("<%=path%>/mobile/serv/znfxAction!doNotNeedSessionAndSecurity_getNewsListByMobileType_new.action?serviceId=<%=serviceId %>&topnews=100"+"&u=<%=accountNum%>",".ui-slider","update",sliderNews,me,"slider");
        ...
        $(document).on('tap','.more',function(){
            ...
            if(target.children('a').attr('alt')>0){
            	temUrl = "<%=path%>/mobile/serv/znfxAction!doNotNeedSessionAndSecurity_getNewsListByMobileType_new.action"+"?category="+target.children('a').attr('alt')+"&serviceId=<%=serviceId %>&start="+start+"&rows="+rows+"&u=<%=accountNum%>";
            }else{
 	...
 	$(document).on('tap','.ui-list > li',function(){
        if($(this).data('href')){
            location.href= $(this).data('href') + "&accountNum=" + accountNumZnfx + "&category=13&sourceFlag=zn";
        }
    });
    $(document).on('tap','.ui-slider-content > li',function(){
        ...
        //初始加载,图片新闻<%=path%>/mobile/serv/znfxAction!doNotNeedSessionAndSecurity_getMobileNewsType_new.action?serviceId=<%=serviceId %>
        ...
        getPageData("<%=path%>/mobile/serv/znfxAction!doNotNeedSessionAndSecurity_getNewsListByMobileType_new.action?serviceId=<%=serviceId %>&topnews=100"+"&u=<%=accountNum%>", ".ui-slider", "", sliderNews, null, "slider");
    });
    </script>
</body>
</html>