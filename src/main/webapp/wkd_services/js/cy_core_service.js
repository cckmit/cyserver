...
//数据加载函数
function getPageData(url,target,status,tpl,me,completeFn) {
	console.log(url);
	$.ajax({
		type: 'GET',
		url: url,
		...
		success: function(data){
			var result = $.tpl(tpl,data);
			if(status == "update" || status == "onload") {
				$(target).html('');
		...
		complete: function(xhr, type){
			...
			if (completeFn == "loadPhotoSwipe") {
				initPhotoSwipeFromDOM('.ui-list-pic');
			}
			if (completeFn == "slider") {
			...
			if (completeFn == "showMore") {
				$(".more").show();
			}
			if (completeFn == "tabNav") {
				...
				tab.on('scrollEnd', function(curPage) {
					// curPage 当前页
					if ($("#content_"+curPage+" .ui-list li").length == 0){
						var _tempUrl = $('#nav_'+curPage).data("href")+"&start=0&rows=10";
						var objTabCurrentHref = $('#nav_'+curPage).data("href");
						if(objTabCurrentHref.indexOf("doNotNeedSessionAndSecurity_getFxjhPageData.action")!=-1){
							getPageData(_tempUrl,"#content_"+curPage+" .ui-list","onload",znFxjhPage,null,"showMore");

							$('#submitid').show();
							setTimeout("$('.more').hide()",800);
							setTimeout("$('.dropload-refresh').hide()",200);
							setTimeout("$('.dropload-update').hide()",200);
							setTimeout("$('.dropload-load').hide()",200);
						}else{
							getPageData(_tempUrl,"#content_"+curPage+" .ui-list","onload",newsList,null,"showMore");
							$('#submitid').hide();
						}
				...
				var tempUrl = $(".ui-tab-nav > li").data("href")+"&start=0&rows=10";
				getPageData(tempUrl,"#content_0 .ui-list","onload",newsList,null,"showMore");
				//console.log(tempUrl);
			}
		}
	});
