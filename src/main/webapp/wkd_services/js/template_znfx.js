...
var sliderNews = 
    ...
    '<li <% if (i == 0){ %>class="current"<% } %>  data-href="../news/newsShow.jsp?newsId=<%=newsList[i].newsId%>"><span style="background-image:url(<%=newsList[i].pic%>)"><h2 class="ui-nowrap"><%=newsList[i].title%></h2></span></li>'+
    '<% } %>'+
    '</ul>' + 
    '<% }else { $(".ui-slider").hide(); }%>';
