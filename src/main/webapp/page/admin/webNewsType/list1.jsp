...
$(function(){
	grid=$('#newsGrid').treegrid({
		url : '${pageContext.request.contextPath}/webNewsType/webNewsTypeAction!dataGridNewsType.action',
//		fitColumns:true,
		idField : 'id',
		treeField : 'name',
		...
		frozenColumns : [ [
		 ...
		 columns:[[
			 ...
			 {field:'deptName',title:'创建组织',width:100,align:'center'},
			    ...
			    {field:'isMain',title:'首页显示',width:80,align:'center',
			    	formatter: function(value,row,index){
						if(row.isMain==0){
							return "×";
			    ...
			    {field:'operator',title:'操作',width:300,
			    		formatter: function(value,row,index){
							var content="";
							<authority:authority authorizationCode="查看网页新闻栏目" userRoles="${sessionScope.user.userRoles}">
			    			content+='<a href="javascript:void(0)" onclick="viewType1('+row.id+')"><img class="iconImg ext-icon-note"/>查看</a>&nbsp;';
			    			</authority:authority>
			    			<authority:authority authorizationCode="编辑网页新闻栏目" userRoles="${sessionScope.user.userRoles}">
			    			content+='<a href="javascript:void(0)" onclick="editType('+row.id+')"><img class="iconImg ext-icon-note_edit"/>编辑</a>&nbsp;';
			    			</authority:authority>
							<authority:authority authorizationCode="删除网页新闻栏目" userRoles="${sessionScope.user.userRoles}">
							content+='<a href="javascript:void(0)" onclick="removeType('+row.id+')"><img class="iconImg ext-icon-note_delete"/>删除</a>&nbsp;';
							</authority:authority>
							if(row.parent_id == 0){
								<authority:authority authorizationCode="查询网页新闻栏目" userRoles="${sessionScope.user.userRoles}">
								content+='<a href="javascript:void(0)" onclick="addType2('+row.id+')"><img class="iconImg ext-icon-note"/>添加子栏目</a>&nbsp;';
								</authority:authority>
							}
			    			return content;
					}}
	...
	function addType() {
		var dialog = parent.modalDialog({
			...
			url : '${pageContext.request.contextPath}/page/admin/webNewsType/add.jsp',
			buttons : [ {
				text : '保存',
	...
	function addType2(id) {
		var dialog = parent.modalDialog({
			title : '新增二级栏目',
			iconCls:'ext-icon-note_add',
			url : '${pageContext.request.contextPath}/page/admin/webNewsType/add.jsp?parent_id='+id,
			buttons : [ {
				text : '保存',
				iconCls : 'ext-icon-save',
				handler : function() {
	...
	function editType(id) {
		var dialog = parent.modalDialog({
			title : '编辑',
			iconCls:'ext-icon-note_add',
			url : '${pageContext.request.contextPath}/page/admin/webNewsType/edit.jsp?id='+id,
			buttons : [ {
				text : '保存',
				iconCls : 'ext-icon-save',
				handler : function() {
	...
	function viewType1(id) {
		var dialog = parent.modalDialog({
			title : '查看',
			iconCls:'ext-icon-note_add',
			url : '${pageContext.request.contextPath}/page/admin/webNewsType/view.jsp?id=' + id
		});
	}
	
	/**--查看子类型--**/
	function viewType2(id) {
		var dialog = parent.modalDialog({
			title : '子栏目',
			iconCls:'ext-icon-note_add',
			url : '${pageContext.request.contextPath}/page/admin/webNewsType/list2.jsp?parent_id=' + id
		});
	}
...
function removeType(id){
	$.messager.confirm('确认', '确定删除吗？', function(r) {
		if (r) {
			$.ajax({
				url : '${pageContext.request.contextPath}/webNewsType/webNewsTypeAction!deleteNewsType.action',
				data : {
					ids : id
				},
				dataType : 'json',
...
/*function removeType(){
	...
	var rows = $("#newsGrid").treegrid('getChecked');
	var ids = [];
	if (rows.length > 0) {
		$.messager.confirm('确认', '确定删除吗？', function(r) {
			if (r) {
			...
			<tr>
				<td>
					<table>
						<tr>
							<td>
							<authority:authority authorizationCode="新增网页新闻栏目" userRoles="${sessionScope.user.userRoles}">
								<a href="javascript:void(0);" class="easyui-linkbutton"
