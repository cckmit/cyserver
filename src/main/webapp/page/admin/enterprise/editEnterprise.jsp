...
//					$('#address').val(str);
        ...
        $(function () {
            ...
            $.ajax({
                url: '${pageContext.request.contextPath}/enterprise/enterpriseAction!getById.action',
                ...
                success: function (result) {
                    ...
                    if (result.id != undefined) {
                        ...
                        $('form').form('load', {
                            ...
                            'enterprise.name': result.name,
                            'enterprise.type': result.type,
                            'enterprise.mainBusiness': result.mainBusiness,
                            'enterprise.address': result.address,
                            'enterprise.website': result.website,
                            'enterprise.recruitEmail': result.recruitEmail,
                            'enterprise.serviceArea': result.serviceArea,
                            'enterprise.linkman': result.linkman,
                            'enterprise.contactNumber': result.contactNumber,
                            'enterprise.slogan': result.slogan,
                            'enterprise.city': result.city,
                            'enterprise.longitude': result.longitude,
                            'enterprise.latitude': result.latitude,
                            'enterprise.locationDesc': result.locationDesc,
                            /*'enterprise.description': result.description,*/
                            'enterprise.summary': result.summary,
                            'enterprise.financingStage': result.financingStage
                        });

                        $('#citySpan').text(result.city);
                        $('#locationDescSpan').html(result.locationDesc);
                        ...
                        createMap("allmap",longitude,latitude,hasVal,1) ;

                        editor.html(result.description);

                        var enterpriseLogo = result.logo;
                        var enterprisePosterPic = result.posterPic;
                        ...
                        if(enterpriseLogo != null && enterpriseLogo != '' && !$('#logo').val()){
                            buildPicDIV(upload_button_name_logo, pic_name_logo, pic_div_name_logo, enterpriseLogo,pic_id_logo);
                            $(upload_button_name_logo).prop('disabled', 'disabled');
                        }
                        if(enterprisePosterPic != null && enterprisePosterPic != '' && !$('#posterPic').val()){

                            buildPicDIV(upload_button_name_poster, pic_name_poster, pic_div_name_poster, enterprisePosterPic, pic_id_poster);
        ...
        function submitForm($dialog, $grid, $pjq)
        {
            ...
            if ($('form').form('validate'))
            {
                ...
                $.ajax({
                    url : '${pageContext.request.contextPath}/enterprise/enterpriseAction!update.action',
                    data : $('form').serialize(),
                    dataType : 'json',
                    success : function(result)
                    {
    ...
    <fieldset>
        ...
        <table class="ta001">
            ...
            <tr id="rg123">
                <th>城市</th>
                <td colspan="3">
                    <span id="citySpan" name="enterprise.city"></span>&nbsp;
            ...
            <tr id="rg223" style="display: none;">
                <th>城市</th>
                <td colspan="3">
                    <input name="enterprise.city" id="resourceArea2" type="hidden"/>
            ...
            <tr>
                <th>图文详情</th>
                <td>
                    <textarea id="description" rows="5" cols="100" name="enterprise.description" style="width: 700px; height: 250px;" ></textarea>
                </td>
            </tr>
            <tr>
                <th>Logo</th>
