...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            new AjaxUpload(button, {
                ...
                onComplete : function(file, response) {
                    ...
                    if (msg.error == 0) {
                        $('#newsPic').append('<div style="float:left;width:180px;"><img src="'+msg.url+'" width="150px" height="150px"/><div class="bb001" onclick="removeNewsPic(this)"></div><input name="serviceNews.pic" value="'+msg.url+'"/></div>');
                ...
                success: function (result) {
                    ...
                    if (result.id != undefined) {
                        $('form').form('load', {
                            ...
                            'serviceNews.serviceId': result.serviceId,
                            'serviceNews.content': result.content,
                            'serviceNews.type': result.type
                        });
                        if(result.channel){
                            url = '${pageContext.request.contextPath}/serviceNewsType/serviceNewsTypeAction!doNotNeedSessionAndSecurity_serviceNewsTypeTree.action?serviceId='+result.serviceId;
                            $('#channel').combobox('reload', url);
                            $('#channel').combobox('setValue', result.channel);
                            $('#lanMu').show();
                        }
                        if(result.pic != undefined && result.pic !=''){
                            $('#newsPic').append('<div style="float:left;width:180px;"><img src="'+ result.pic +'" width="150px" height="150px"/><div class="bb001" onclick="removeNewsPic(this)"></div><input type="hidden" name="serviceNews.pic" value="'+result.pic+'"/></div>');
                        }
        ...
        function submitForm($dialog, $grid, $pjq)
        {
            ...
            {
                $.ajax({
                    url : '${pageContext.request.contextPath}/serviceNews/serviceNewsAction!updateServiceNews.action',
                    data : $('form').serialize(),
                    dataType : 'json',
                    success : function(result)
                    {
