    ...
    <script type="text/javascript">
        ...
        $(function () {
            eventGrid = $('#eventGrid').datagrid({
                url: '${pageContext.request.contextPath}/event/eventAction!getEventBoardx.action?id='+$('#eventId').val(),
                nowrap: false,
                fit: true,
                border: false,
                fitColumns: true,
					...
					},
					{
	                    ...
	                    formatter: function (value, row) {
							...
							if(row.status == '0' || row.status == '1' || row.status == '2') {
	                        	<authority:authority authorizationCode="处理校友会活动花絮" userRoles="${sessionScope.user.userRoles}">
									str += '<a href="javascript:void(0)" onclick="del(' + row.id + ');"><img class="iconImg ext-icon-note_delete"/>删除</a>&nbsp;';
								</authority:authority>
	                        } else if(row.status == '4'){
	                        	<authority:authority authorizationCode="处理校友会活动花絮" userRoles="${sessionScope.user.userRoles}">
		...
		var commentFun = function (id) {
            var dialog = parent.modalDialog({
                title: '查看评论',
                iconCls: 'ext-icon-note',
                url: '${pageContext.request.contextPath}/page/alumni/event/viewEventBoardComment.jsp?id=' + id
            });
        };
        ...
        function handleEventBoard(boardId, status){			
			$.ajax({
				url : '${pageContext.request.contextPath}/event/eventAction!handleEventBoardx.action',
				data : {
					id : boardId,
					handleStatus: status
				},
