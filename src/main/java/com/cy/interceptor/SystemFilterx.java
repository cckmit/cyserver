...
public class SystemFilterx implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (user == null) {
			...
			request.getRequestDispatcher("/error/timeOutx.jsp").forward(request, response);
			return;
		}
		if (hsr.getServletPath().equals("/druid")) {
			if (user.getRole() != null && user.getRole().getSystemAdmin() == 1) {
				...
				request.getRequestDispatcher("/error/reject.jsp").forward(request, response);
			}
		} else {
			chain.doFilter(hsr, res);
		}
