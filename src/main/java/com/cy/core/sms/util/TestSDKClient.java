package com.cy.core.sms.util;

import java.util.List;

import com.cy.core.sms.manager.SingletonClient;
...
public class TestSDKClient {
    ...
    public static String password="265782";// 密码,请通过亿美销售人员获取
...
//		Client client2=new Client(softwareSerialNo,password);
    ...
    public static void testSendSMS() {
        try {
            int i=SingletonClient.getClient().sendSMS(new String[] {"13667263323"}, "【华中科技大学校友总会】注册码：364747。若非您本人操作，请忽略此短信! ","",5);//带扩展码
    ...
    public static void testsSendScheduledSMS() {
        try {
            int i=SingletonClient.getClient().sendScheduledSMSEx(new String[] {"13667263323"}, "定时短信", "20111206160000","GBK");
    ...
    public static void testsSendSMSAddMessageId() {
        try {
            int i=SingletonClient.getClient().sendSMSEx(new String[] {"13667263323"}, "即时短信，带短信ID", "", "GBK", 5, 1234567);
            System.out.println("testsSendScheduledSMS====="+i);
        } catch (Exception e) {
            e.printStackTrace();
        }
