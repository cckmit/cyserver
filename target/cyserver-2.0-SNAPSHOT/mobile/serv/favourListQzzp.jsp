...
<script>
    ...
    Zepto(function($){
  	 ...
  	 //页面加载
    getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>",".thread-list","onload",appealTplQzzp,null,"loadPhotoSwipe");
    ...
    var dropload = $('.inner').dropload({
      ...
      loadUpFn : function(me){
        getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>&cyServ.currentRow=0&cyServ.incremental=" + $(".thread-list > li").length,".thread-list","update",appealTplQzzp,me);
      },
      loadDownFn : function(me){
  		...
  		{
  			getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>&cyServ.currentRow=" + $(".thread-list > li").length,".thread-list","more",appealTplQzzp,me);
  		}
      }
    });
  ...
  $(document).on('tap','.show-bot-menu',function(){
    ...
    $(document).on('tap','.choose-city .ui-label',function(){
      ...
      getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=" + city,".thread-list","onload",appealTplQzzp,null,"loadPhotoSwipe");
