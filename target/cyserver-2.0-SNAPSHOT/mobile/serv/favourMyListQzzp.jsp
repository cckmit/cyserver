...
String category = request.getParameter("category");//category:3 求职招聘
...
<head>
    ...
    <style>
        .labelheader_xmhz{font-size:14px;color:#4AA7DE;background: #ffffff;border-top: 1px solid #F9F9FB;height: 25px;padding:10px 0px 0px 15px;margin: 15px 0px 0px 0px;}
    </style>
</head>
<body>
...
<section class="ui-container my-post">
    <div class="ui-tab">
        <ul class="ui-tab-nav ui-border-b">
            <li data-href="favourListQzzp.jsp?isWhat=1&category=<%= category %>&accountNum=<%= accountNum %>" <% if("1".equals(isWhat)){%>class="current"<%}%>>全部</li>
            <li data-href="favourMyListQzzp.jsp?isWhat=2&category=<%= category %>&accountNum=<%= accountNum %>" <% if("2".equals(isWhat)){%>class="current"<%}%>>我的</li>
        </ul>
        <ul class="ui-tab-content" style="width:200%">
            <li class="current">
                <div class="labelheader_xmhz">
                    <img src="../img/icon_comments.png" style="margin:0px 5px 0px 0px;"/>我的招聘
                </div>
                <div class="thread-list" id="myPost" style="margin-top: 0px;">
                </div>

                <div class="labelheader_xmhz"><img src="../img/Star4.png" style="margin:0px 5px 0px 0px;"/>我的收藏</div>
...
<script src="../js/cy_core_qzzp.js"></script>
...
<script>
    ...
    function added() {
        window.location.href = B.serverUrl + "/mobile/serv/servHelpCreateQzzp.jsp?accountNum=" + accountNum + "&category=" +category+ "&isWhat=" + isWhat;
    }

    //页面加载
    getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>","#myPost","onload",appealTplQzzp2,null,"loadPhotoSwipe");
    getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%>&cyServ.viewType=favorite<%= listExtensionParameters %>&cyServ.region=<%=region%>","#myFavorites","onload",appealTplQzzp2,null,"loadPhotoSwipe");
    ...
    $(document).on('tap','.more',function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                var result=$.tpl(appealTplQzzp2,data);
    ...
    (function (){
        ...
        tab.on('scrollEnd', function(curPage) {
            if($('#myPost').children().length == 0) {
                getPageData("mobServAction!doNotNeedSessionAndSecurity_getServList.action?cyServ.category=<%=category%><%= listExtensionParameters %>&cyServ.region=<%=region%>","#myPost","",appealTplQzzp2,null,"loadPhotoSwipe");
            }
