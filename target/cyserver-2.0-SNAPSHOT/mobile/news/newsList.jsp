        ...
        function sigleCategory(){
            var dropload = $('.inner').dropload({
                ...
                loadUpFn : function(me){
                    // 查询顶级栏目（无子栏目）或唯一子栏目下的轮播新闻【category=顶级栏目ID】
                    ...
                    // 查询顶级栏目（无子栏目）或唯一子栏目下的列表新闻【category=顶级栏目ID（无子栏目）或唯一子栏目ID】
                    getPageData("<%=path%>/mobile/news/newsAction!doNotNeedSessionAndSecurity_getNewsListByMobileType.action?category="+category+"&start=0&rows=10"+"&u=<%=accountNum%>",".ui-list","update",newsList,me);
                }
            });
        }
        ...
        function multiCategory(){
            var dropload = $('.inner').dropload({
                ...
                loadUpFn : function(me){
                    ...
                    getPageData("<%=path%>/mobile/news/newsAction!doNotNeedSessionAndSecurity_getNewsListByMobileType.action?category=<%=category %>&topnews=100"+"&u=<%=accountNum%>",".ui-slider","update",sliderNews,me,"slider");
                    // 查询顶级栏目下所有子栏目生成页签
                    getPageData($('.ui-tab-nav .current').data('href')+"&start=0&rows=10", ".ui-tab-content .current .ui-list", "update", newsList, me, "showMore");
                }
            });
        }
