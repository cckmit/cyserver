...
function addCookie(objName, objValue, objHours, objPath) {
	...
	if(objPath != null && objPath != '' && objPath != 'null') {
		str += ";path="+objPath ;
	} else {
		str += ";path=/";
	}
...
function logout() {
	...
	window.location.href=B.clientUrl+"index.html";
...
function setLeftNavDate() {
	//从cookie中获取个人头像
	var pPic = getCookie("picture");
	// if(!(pPic && pPic != '' && is_website(pPic)))
		// alert(pPic)
	if(!pPic || pPic=="" || !is_website(pPic))
		pPic = B.clientUrl+'image/user-avatar.png';
	var avatarStr =
...
function reBuildPic() {
	...
	for(i=0;i<AllImages.length;i++)
	{
		...
		{
			this.src="image/users-avatar.png";
		}
	}
}