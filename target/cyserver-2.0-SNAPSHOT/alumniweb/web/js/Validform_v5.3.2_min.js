...
(function (d, f, b) {
    ...
    a.util = {
        dataType: {
            ...
            e: /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
            "zh2-4" : /^[\u4E00-\u9FA5\uf900-\ufa2d]{2,4}$/, //jiangling 自定义 中文
            zeName: /(^[\u4e00-\u9fa5]{1}[\u4e00-\u9fa5\.·。]{0,8}[\u4e00-\u9fa5]{1}$)|(^[a-zA-Z]{1}[a-zA-Z\s]{0,8}[a-zA-Z]{1}$)/,//jiangling 自定义 中文
            url: /^(\w+:\/\/)?\w+(\.\w+)+.*$/
            ...
            o.find("[datatype]").each(function () {
                if (l == 2) {
                    if (d(this).parent().next().find(".Validform_checktip").length == 0) {
                        d(this).parent().next().append("<span style='color:#FF0000' class='Validform_checktip' />");
                        d(this).siblings(".Validform_checktip").remove()
                    }
                } else {
                    if (l == 3 || l == 4) {
                        if (d(this).siblings(".Validform_checktip").length == 0) {
                            d(this).parent().append("<span style='color:#FF0000' class='Validform_checktip' />");
