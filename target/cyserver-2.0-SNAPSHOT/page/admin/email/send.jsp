...
<html>
  <head>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for(var i=0;i<list.length;i++){
                    var obj = list[i];
                    if(list.length == 1){
                        emailNumText = emailNumText + obj.email;
                        userNameText = userNameText + obj.userName ;
	...
	{
		var emailType=$("input[name='emailType']:checked").val();
		var emailNumText = $("#toAddress").val();
		if(Validate.isNull(emailNumText)){
		...
		var emailList = emailNumText.split(",");
		for(var i=0;i<emailList.length;i++){
			var email = emailList[i];
			if(!Validate.isEmail(email)){
				parent.$.messager.alert('错误', email+'邮箱格式非法', 'error');
				return;
			}
		}
		
		if($('#ccAddress')[0]!=undefined&&!Validate.isNull($('#ccAddress').val())){
			var emailList = $('#ccAddress').val().split(",");
			for(var i=0;i<emailList.length;i++){
				var email = emailList[i];
				if(!Validate.isEmail(email)){
					parent.$.messager.alert('错误', '抄送'+email+'邮箱格式非法', 'error');
					return;
				}
			}
		}
		...
		if($('#bccAddress')[0]!=undefined&&!Validate.isNull($('#bccAddress').val())){
			var emailList = $('#bccAddress').val().split(",");
			for(var i=0;i<emailList.length;i++){
				var email = emailList[i];
				if(!Validate.isEmail(email)){
					parent.$.messager.alert('错误', '密送'+email+'邮箱格式非法', 'error');
					return;
				}
			}
		}
		...
		{
			parent.parent.$.messager.confirm('确认', '确定发送'+emailList.length+'条邮件吗？', function(r)
			{
				if (r)
				{
					$.ajax({
						url : '${pageContext.request.contextPath}/page/admin/email/emailAction!save.action',
						data : $('form').serialize(),
						dataType : 'json',
		...
		<tr>
			...
			<td > 
				<input name="email.fromAddress" class="easyui-validatebox"
					...
					maxlength="100" value="<%=email_account%>" readonly="readonly"/>
			</td>
		</tr>
		<%-- <tr>
			<th>
