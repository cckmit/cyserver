...
$(function(){
	grid=$('#newsGrid').datagrid({
		url : '${pageContext.request.contextPath}/webNewsType/webNewsTypeAction!dataGridNewsType.action?type.parent_id=<%=parent_id %>',
		fit : true,
		border : false,
		fitColumns : true,
		striped : true,
		...
		idField : 'id',
		 columns:[[  
			    ...
			    {field:'isMain',title:'首页显示',width:100,align:'center',
			    	formatter: function(value,row,index){
						if(row.isMain==0){
							return "×";
...
function searchNews(){
	...
	function addType() {
		var dialog = parent.modalDialog({
			...
			url : '${pageContext.request.contextPath}/page/admin/webNewsType/add.jsp?parent_id=<%=parent_id%>',
			buttons : [ {
				text : '保存',
		...
		if (rows.length > 0) {
			$.messager.confirm('确认', '确定删除吗？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/webNewsType/webNewsTypeAction!deleteNewsType.action?parent_id=<%=parent_id %>',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
