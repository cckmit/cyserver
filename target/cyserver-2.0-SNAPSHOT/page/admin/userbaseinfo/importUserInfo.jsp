							...
							*/
							if(result.msg=="")
							{
								...
								var MyInterval1 = window.setInterval(function () {
									$.ajax({
										url : '${pageContext.request.contextPath}/userBaseInfo/userBaseInfoAction!importState.action',
										dataType : 'json',
										success : function(result) {
											if ( result.state == 0 ) {
											...
											{
												parent.parent.$.messager.progress('close');
												if(result.now!=undefined && result.total!=undefined) {
                                                    parent.parent.$.messager.progress({
                                                        text: '正在整理数据,请稍后...已导入进度:' + result.now + '/' + result.total
                                                    });
                                                }else{
                                                    parent.parent.$.messager.progress({
                                                        text: '正在整理数据,请稍后...'
												...
												parent.parent.$.messager.progress('close');
												parent.parent.$.messager.progress({
													text : '正在导入数据,请稍后...已导入进度:' + result.now + '/' + result.total
												});
											}
										}
									});
