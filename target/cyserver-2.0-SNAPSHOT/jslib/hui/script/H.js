...
; ! function (factory) {
    if (typeof require === 'function' && typeof exports === 'object' && typeof module === 'object') {
        ...
        factory(window['H'] = {
            ...
            // 支付成功弹窗
            dialogCoreForPaySuccuss: function (type, title, msg, buttons) {

                // console.log("提示信息:"+"type=="+type+" | msg= "+msg+"| title="+title+"buttons="+buttons);


                var that = this;
                title = title ? title : "捐赠成功";
                var clientWidth = window.document.body.clientWidth;

                var contentHtml = "";
                contentHtml +=
                    '<div class="H-dialog H-theme-background-color-white animated zoomIn H-border-radius-9" ' +
                    'style="-webkit-animation-duration: 0.3s; animation-duration: 0.3s;width: ' + (clientWidth - 50) + 'px; min-height: 80px; position:relative">';
                contentHtml += '<div class="H-text-align-center H-padding-vertical-top-6" style="margin: 8px;">' +
                    '<div class="H-dialog-title H-padding-10 H-font-size-16 H-line-height-normal">' + title + '</div>'+
                    '<span class="H-icon H-display-block H-horizontal-center">'+
                    '<img src="'+B.serverUrl+'/foundation/img/tupian-@3x.png" style="width: 200px;" />'+
                    '</span>'+
                '</div>';
                contentHtml += '<div id="messageWin" ' +
                    'class="H-dialog-content H-padding-10 H-word-break-break-all H-font-size-13 H-theme-font-color-666 H-text-align-center" style="margin-bottom: 27px;">' + msg + '</div>';
                    ...
                    for (var i = 0; i < buttons.length; i++) {
                        if (i == (buttons.length - 1) && buttons.length > 1) {
                            contentHtml +=
                                ...
                                'H-center-all H-touch-active" style="border: 1px solid #fa9641;margin: 20px;color: #fa9641;">' + buttons[i] + '</div>';
                        }
                        else {
                            contentHtml +=
                ...
                contentHtml += '</div>';
                contentHtml += '<div class="close-icon" style="position: absolute;text-align: center; width: 100%;height:50px;line-height: 50px; bottom: -70px"><img style="width:20px" src="'+B.serverUrl+'/foundation/img/quxiao@3x.png"/></div>'
                contentHtml += '</div>';
                ...
                var scrollObj = that.getScroll();
                that.D(".H-dialog-area").style.height = window.innerHeight + scrollObj.t + "px";
                that.D(".H-dialog").style.marginTop = scrollObj.t-80 + "px";
            ...
            // 支付失败弹窗
            dialogCoreForPayFail: function (type, title, msg, buttons) {


                var that = this;
                title = title ? title : "支付失败";
                var clientWidth = window.document.body.clientWidth;
            ...
            //支付成功弹窗
            alertTipForPaySuccess: function (callback, msg, title, buttons, closeback) {
                ...
                // 生成HTML
                that.dialogCoreForPaySuccuss("text", title, msg, buttons);  // that function add by jiangling
                ...
                that.on(that.Ds('.close-icon'), "touchend", function (e) {
                    that.addClass(that.D('.H-dialog-area'), "H-display-none-important");
                    //that.D('.H-dialog-area').remove();
                    if(that.isFunction(closeback)){
                        closeback();
                    }
                });
            ...
            //支付失败弹窗
            alertTipForPayFail: function (callback, msg, title, buttons) {
                // console.log("callback="+callback +"|msg="+msg + "| title="+title +"|buttons="+buttons);
                var that = this;
                ...
                // 生成HTML
                that.dialogCoreForPayFail("text", title, msg, buttons);  // that function add by jiangling
